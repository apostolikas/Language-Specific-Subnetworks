...
def compute_importance(
    model, eval_dataloader, device, compute_importance=True, head_mask = None, mlp_mask=None
):
...
def mask_model(args, model, eval_dataloader, device):
    ...
    """
    head_importance, mlp_importance, preds, labels = compute_importance(model, eval_dataloader, device=device)
    ...
    while current_score >= original_score * args.masking_threshold:
        ...
        head_importance, mlp_importance, preds, labels = compute_importance(
            model, eval_dataloader, device=device, head_mask=new_head_mask, mlp_mask=new_mlp_mask
        )
    ...
    while current_score >= original_score * args.masking_threshold:
        ...
        # Compute metric and head,mlp importance again
        head_importance, mlp_importance, preds, labels = compute_importance(
            model, eval_dataloader, device=device, head_mask=new_head_mask, mlp_mask=mlp_mask
        )
    ...
    while current_score >= original_score * args.masking_threshold:            
        ...
        # Compute metric and head,mlp importance again
        head_importance, mlp_importance, preds, labels = compute_importance(
            model, eval_dataloader, device=device, head_mask=head_mask, mlp_mask=new_mlp_mask
        )
...
def prune_model(args, model, eval_dataloader, device, head_mask, mlp_mask):
    ...
    _, _, preds, labels = compute_importance(
        model, eval_dataloader, device=device, compute_importance=False, head_mask=head_mask, mlp_mask=mlp_mask
    )
    preds = np.argmax(preds, axis=1)
    ...
    before_time = datetime.now()
    _, _, preds, labels = compute_importance(
        model, eval_dataloader, device=device, compute_importance=False, head_mask=None, mlp_mask=None
    )
    preds = np.argmax(preds, axis=1)
    ...
    print(f"Pruning: score with masking: {score_masking:2f} score with pruning: {score_pruning:.2f}")
    print(f"Pruning: speed ratio (original timing / new timing): {original_time / new_time * 100:.2} percents")
