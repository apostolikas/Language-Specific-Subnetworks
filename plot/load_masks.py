    ...
    for lang in LANGUAGES:
        ...
        for task in TASKS:
            ...
            combined_iterable = zip(
                *[masks_dict[lang][task][seed].flatten().tolist() for seed in SEEDS])
        ...
        for task_id1, task_id2 in itertools.combinations([k for k in range(len(TASKS))], r=2):
            overlap_lang[task_id1,
                         task_id2] = len(intersection(heads_ids[task_id1], heads_ids[task_id2]))
        plot_lower_triangular_matrix(overlap_lang,
                                     TASKS,
                                     TASKS,
                                     f'super_survivors_{lang}',
                                     'TASK',
                                     'TASK')
